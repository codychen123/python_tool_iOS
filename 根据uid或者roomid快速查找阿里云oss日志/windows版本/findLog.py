...
main_path = os.getcwd()
abs_path = 'D:/python/查日志/findLog.py'
...
def getZip():
	...
	for object_dic in objectList:
		...
		zip_name = str(object_dic["name"]).replace('*','^^')
		...
		if int(abs_result_time) < load_timestamp_disparity:
			...
			zip_download_url = '%s/%s'%(aliyun_get_zip_download_url, zip_path)
			r = requests.get(zip_download_url)
			# download_path = os.path.join()
			# urllib.request.urlretrieve(zip_download_url, '%s'%(zip_name))  2388999163930
			dowload_path = '%s\%s'%(main_path, zip_name)
...
def main():
	# r = requests.get('http://uxin-zb-picture.oss-cn-shenzhen.aliyuncs.com/app_log/2020-01-04/2388999163930*PBAM00*8.1.0*1578097876261.zip')
	# with open('^^1578097876261.zip', "wb") as code:
	# 	code.write(r.content)
	get_parameter()
	deleteOldDir()
	beginInput()
